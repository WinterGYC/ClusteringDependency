...
void printRes(vector<vector<string> > result) {
    ...
    for ( int i=0 ; i<n ; i++, cout << endl  ) {
        for ( int j=0 ; j<m ; j++) if ( j==3 || j==4 || j==10 ) {
            cout << result[i][j] << " ### ";
        }
    }
}
...
int main() {
    ...
    // pre-processing, eliminate "-" from dates
    for ( int i=0 ; i<m ; i++ ) {
        for ( int j=0 ; j<n ; j++ ) {
            int pos = result[i][j].find("-");
            while ( pos != -1 ) {
                result[i][j].erase(result[i][j].begin()+pos);
                pos = result[i][j].find("-");
            }
        }
    }
    ...
    // set parameter
    indLHS = 3;
    int aa[12]={3,4,6,7,8,9,10,11,12,13,14};
    vector<int> candi(aa,aa+11);
    vector<int> indRHS;
