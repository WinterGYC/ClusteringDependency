		...
		while ( !newValidated.empty() ) {
			...
			if ( nodeValid[p]==1 ) {
				...
				for ( long long i=0 ; i<validRHS.size() ; i++ ) {
					...
					if ( (validRHS[i] & p)==0 && nodeValid[validRHS[i] | p] == 0 ) {
				...
				for ( long long i=0 ; i<neighbours[p].size() ; i++ ) {
					...
					if ( neighbours[p][i] > p && nodeValid[neighbours[p][i]]==0 ) { // as we are going DOWN
				...
				for ( long long i=0 ; i<validRHS.size() ; i++ ) {
					...
					if ( (validRHS[i]<p ) && cover(p, validRHS[i]) && nodeValid[p - validRHS[i]]==0 ) {
						// found a new valid node!
				...
				// then apply rules with OD/FD
				for ( long long i=0 ; i<neighbours[p].size() ; i++ ) {
					if ( neighbours[p][i] < p && nodeValid[neighbours[p][i]]==0 ) { // as we are going UP
						testFile << "cost 1: " << neighbours[p].size() << endl;
